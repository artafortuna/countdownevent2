...
if (savedTheme === 'dark') {
    ...
    if(themeIcon) themeIcon.className = "fa-solid fa-sun text-[14px] text-pasir-500";
} else {
    htmlElement.classList.remove('dark');
    if(themeIcon) themeIcon.className = "fa-solid fa-moon text-[14px] text-pasir-600";
}
...
function closeSearch() {
    ...
    if (htmlElement.classList.contains('dark')) {
        themeBtnIcon.className = "fa-solid fa-sun text-[14px] text-pasir-500";
    } else {
        themeBtnIcon.className = "fa-solid fa-moon text-[14px] text-pasir-600";
    }
...
function openModal(type, title, message, onConfirm) {
    if (type === 'alert') {
        modalIcon.className = "w-12 h-12 rounded-full flex items-center justify-center mb-2 shadow-[0_0_15px_rgba(34,211,238,0.5)] bg-cyan-100 dark:bg-cyan-900/50 text-cyan-600 dark:text-cyan-400 border border-cyan-300 dark:border-cyan-700";
        modalIcon.innerHTML = '<i class="fa-solid fa-bell text-lg"></i>';
        modalActions.innerHTML = `
            <button onclick="closeModal()" class="w-full bg-gradient-to-r from-pantai-500 to-blue-600 text-white font-black py-2 rounded-xl transition-all shadow-[0_0_10px_rgba(34,211,238,0.5)] active:scale-[0.98] text-xs border border-cyan-400">
                Tutup Peringatan
            </button>
        `;
    } else if (type === 'confirm') {
        modalIcon.className = "w-12 h-12 rounded-full flex items-center justify-center mb-2 shadow-[0_0_15px_rgba(244,63,94,0.5)] bg-rose-100 dark:bg-rose-900/50 text-rose-600 dark:text-rose-400 border border-rose-300 dark:border-rose-700";
        modalIcon.innerHTML = '<i class="fa-solid fa-triangle-exclamation text-lg"></i>';
        modalActions.innerHTML = `
            <button onclick="closeModal()" class="flex-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-black py-2 rounded-xl transition-all shadow-sm text-xs">
                Batal
            </button>
            <button id="btn-confirm-action" class="flex-1 bg-gradient-to-r from-rose-500 to-red-600 text-white font-black py-2 rounded-xl transition-all shadow-[0_0_10px_rgba(244,63,94,0.5)] active:scale-[0.98] text-xs border border-rose-400">
                Ya, Hapus
            </button>
        `;
...
function setFilter(filter) {
    ...
    ['all', 'active', 'passed'].forEach(f => {
        ...
        if (f === filter) {
            btn.className = "flex-1 py-1.5 rounded-lg text-xs font-black bg-gradient-to-r from-pantai-500 to-blue-600 text-white border border-cyan-300 shadow-sm transition-all";
        } else {
            btn.className = "flex-1 py-1.5 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-cyan-100 dark:hover:bg-slate-800 transition-all";
        }
    });
    renderEvents();
}
...
function generateEventCardHTML(evt) {
    ...
    return `
        <div class="p-3 sm:p-3.5 rounded-[1rem] bg-gradient-to-br ${evt.theme} text-white shadow-md relative overflow-hidden flex flex-col justify-between min-h-[120px] border border-white/30 backdrop-blur-lg group hover:scale-[1.02] transition-transform" data-target="${evt.targetDate}" data-id="${evt.id}">
            ...
            <div class="relative z-10 flex flex-col h-full justify-between">
                <div>
                    <div class="flex justify-between items-start gap-1">
                        <span class="text-[9px] sm:text-[10px] uppercase font-black tracking-widest px-2 py-0.5 rounded-full bg-black/30 text-white backdrop-blur-md shadow-inner border border-white/30 truncate max-w-[65%]">
                            ${evt.category}
                        </span>
                    ...
                    <h4 class="text-xs sm:text-sm font-mewah font-black text-white mt-2.5 leading-tight tracking-wide pr-1 drop-shadow-md line-clamp-2">${evt.name}</h4>
                ...
                <div class="grid grid-cols-4 gap-0.5 text-center mt-2.5 bg-black/20 p-1 sm:p-1.5 rounded-xl backdrop-blur-md border border-white/30 shadow-inner countdown-container">
                    <div class="flex flex-col">
                        <div class="text-xs font-black tracking-tighter days-val text-white drop-shadow-md">00</div>
                        <div class="text-[7px] uppercase font-black tracking-widest text-white/80">Hari</div>
                    </div>
                    <div class="flex flex-col">
                        <div class="text-xs font-black tracking-tighter hours-val text-white drop-shadow-md">00</div>
                        <div class="text-[7px] uppercase font-black tracking-widest text-white/80">Jam</div>
                    </div>
                    <div class="flex flex-col">
                        <div class="text-xs font-black tracking-tighter minutes-val text-white drop-shadow-md">00</div>
                        <div class="text-[7px] uppercase font-black tracking-widest text-white/80">Mnt</div>
                    </div>
                    <div class="flex flex-col">
                        <div class="text-xs font-black tracking-tighter seconds-val text-white drop-shadow-md">00</div>
                        <div class="text-[7px] uppercase font-black tracking-widest text-white/80">Det</div>
                    </div>
                </div>
            </div>

            <div class="absolute inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center text-center p-2.5 transition-opacity duration-500 opacity-0 pointer-events-none passed-overlay z-20 rounded-[1rem]">
                <div class="bg-gradient-to-tr from-cyan-400 to-blue-500 text-white w-8 h-8 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(34,211,238,0.6)] mb-1.5 border border-white/50">
                    <i class="fa-solid fa-check text-xs"></i>
                </div>
                <div class="text-[11px] sm:text-xs font-black text-white tracking-wide drop-shadow-md line-clamp-1">${evt.name}</div>
                <div class="text-[8px] font-black uppercase tracking-widest text-cyan-300 mt-1 bg-cyan-500/20 px-2 py-0.5 rounded-full border border-cyan-400/30">Acara Tiba 🌊</div>
                
                <div class="flex flex-col sm:flex-row justify-center items-center mt-2 gap-1.5 w-full px-1">
                    ${isBirthday ? `
                        <button onclick="ulangTahunAcara('${evt.id}')" class="w-full text-[9px] bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white py-1 rounded-lg transition font-black pointer-events-auto active:scale-95 shadow-sm border border-cyan-300">
                            Tahun Depan
                        </button>
                    ` : ''}
                    <button onclick="deleteEvent('${evt.id}')" class="w-full text-[9px] bg-white/10 hover:bg-rose-500/40 text-white hover:text-rose-200 py-1 rounded-lg transition font-black pointer-events-auto active:scale-95 border border-white/30 hover:border-rose-400 backdrop-blur-md">
                        Hapus
                    </button>
                </div>
            </div>
